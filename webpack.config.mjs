...
const userConfig = {
	extraScripts: [],
	...(fs.pathExistsSync(userConfigPath) ? JSON.parse(fs.readFileSync(userConfigPath, 'utf8')) : {}),
};
...
function createPluginArchive(sourceDir, destPath) {
	const distFiles = globSync(`${sourceDir}/**/*`, { nodir: true, windowsPathsNoEscape: true })
...
function resolveExtraScriptPath(name) {
	...
	// Standalone window scripts need browser-compatible output (not CommonJS)
	const isStandaloneScript = name.includes('dialog/window/') || name === 'mobile-webview.ts';
	...
	return {
		...
		output: {
			...
			path: distDir,
			...(isStandaloneScript
				? { libraryTarget: 'window', library: nameNoExt.replace(/\//g, '_') }
				: { library: 'default', libraryTarget: 'commonjs', libraryExport: 'default' }),
		},
	};
}
...
function buildExtraScriptConfigs(userConfig) {
	...
	for (const scriptName of userConfig.extraScripts) {
		...
		const isStandaloneScript = scriptName.includes('dialog/window/') || scriptName === 'mobile-webview.ts';
		output.push({
			...extraScriptConfig,
			entry: scriptPaths.entry,
			output: scriptPaths.output,
			...(isStandaloneScript ? { target: 'web' } : {}),
		});
	}
